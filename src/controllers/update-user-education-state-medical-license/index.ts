...
const UpdateUserEducationStateMedicalLicense = {
  async handle(req: Request, res: Response) {
    try {
      const { state_medical_license_id, state_medical_license, state_medical_license_url, state_medical_license_public_id } =
        req.body?.input?.input || req?.body?.input || req?.body;


      if (state_medical_license_public_id) {
        try {
          await Cloudinary.cloudinaryDelete(state_medical_license_public_id);
        } catch (error) {
