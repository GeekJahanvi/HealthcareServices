...
const UpdateUserEducation = {
  async handle(req: Request, res: Response) {
    try {
      const { fellow_ship_2_id, fellow_ship_2, fellow_ship_2_url, fellowship_2_public_id } = req.body?.input?.input || req?.body?.input || req?.body;


      if (fellowship_2_public_id) {
        try {
          await Cloudinary.cloudinaryDelete(fellowship_2_public_id);
        } catch (error) {
